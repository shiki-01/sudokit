<script lang="ts">
	...
	import { memoize, now } from 'lodash-es';
	...
	const type = $level === 'Easy' ? 'easy' : $level === 'Normal' ? 'medium' : $level === 'Hard' ? 'hard' : 'expert';
	...
	const bg = (selected: [number, number], row: number, col: number): string => {
		...
		if (getNum(sudoku, selRow, selCol) === getNum(sudoku, row, col) && getNum(sudoku, row, col) !== '') return Color($themeColor, 'bg', '200');
		if (selRow === row || selCol === col || (getBlock(selRow) === getBlock(row) && getBlock(selCol) === getBlock(col))) return Color($themeColor, 'bg', '200/50');
		return Color($themeColor, 'bg', '50');
	};

	const isCorrect = (num: string, row: number, col: number): boolean => {
		return num === getNum(answer, row, col) && getNum(sudoku, row, col) !== '';
	};

	const isRowCorrect = (row: number): boolean => {
		return Array.from({ length: 9 }, (_, i) => getNum(sudoku, row, i)).every((num, i) => num === getNum(answer, row, i));
	};

	const isColCorrect = (col: number): boolean => {
		return Array.from({ length: 9 }, (_, i) => getNum(sudoku, i, col)).every((num, i) => num === getNum(answer, i, col));
	};

	const isBlockCorrect = (blockRow: number, blockCol: number): boolean =>
		Array.from({ length: 9 }, (_, i) => getNum(sudoku, blockRow + Math.floor(i / 3), blockCol + (i % 3))).every((num, i) => num === getNum(answer, blockRow + Math.floor(i / 3), blockCol + (i % 3)),
			(blockRow: number, blockCol: number) => `${blockRow},${blockCol}`
		);

	const DorL = () => {
		return $theme === 'light' ? 'highlight' : 'lowlight';
	};
	...
	const checkCompletion = () => {
		...
		const completedTypes: { type: 'row' | 'col' | 'block', index: number, done: boolean, correct: boolean }[] = [];
		...
		if (completedTypes.length === 27) {
			completed = [{ type: 'all', index: 0, done: false, correct: true }];
		} else {
	...
	const handleAnimationEnd = (event: Event) => {
		...
		completed = completed?.map(c => c.index === index ? { ...c, done: true } : { ...c, done: true }) || [];
	};

	const handleCellClick = (row: number, col: number) => {
		if (keepMode && selectedNumber) {
			console.log(getCell(row, col).display, isNumberFullyUsed($sudokuStore.sudoku, selectedNumber));
...
{#if completed && completed.some(c => c.type === 'all' && c.correct)}
	<div
		...
		<Confetti x={[-0.5, 1.5]} y={[-0.5, 1.5]} amount={80}
							colorArray={["#ffbe0b", "#fb5607", "#ff006e", "#8338ec", "#3a86ff"]} />
		<Confetti x={[-1.5, 0.5]} y={[-0.5, 1.5]} amount={80}
							colorArray={["#ffbe0b", "#fb5607", "#ff006e", "#8338ec", "#3a86ff"]} />
	</div>
	<div class="w-screen h-screen fixed top-0 left-0 flex justify-center items-center z-50 backdrop-blur-sm {Color($themeColor,'bg','900/20')}">
		<div class="w-[250px] h-[300px] flex flex-col gap-6 justify-center items-center rounded-xl shadow-xl {Color($themeColor,'bg','50')} {Color($themeColor,'shadow','400/40')}">
			<p class="font-bold border px-4 py-1 rounded-md {Color($themeColor,'border','800')}">Result</p>
			<p>Time: {time[0]}:{time[1]}:{time[2]}</p>
			<p>Missed: {$sudokuStore.missing}</p>
			<div>
				<button
					class="flex flex-row gap-2 p-4 pb-2 border-b-2 {Color($themeColor,'border','300')} {Color($themeColor,'text','800')}"
...
<div
	class="w-screen h-screen fixed top-0 left-0 flex justify-center items-center backdrop-blur-sm z-40 transition-opacity {Color($themeColor,'bg','900/50')} {isStopped ? 'opacity-100' : 'opacity-0 pointer-events-none'}"
/>
...
>
	<div class="flex flex-col justify-center items-center">
		...
		>
			{#each Array(3) as _, boxRow}
				{#each Array(3) as _, boxCol}
					<div
						class="w-fit h-fit grid grid-cols-3 grid-rows-3 overflow-hidden border-2 {Color($themeColor,'border','300')}
            {completed && completed.some(c => !c.done && c.type === 'block' && Math.floor(c.index / 3) === boxRow && c.index % 3 === boxCol) ? DorL() : ''}"
                    ...
                    {isCorrect(cell.number, row, col) ? Color($themeColor,'text','800') : Color('red','text','400',true)}
                    {Color($themeColor,'border','300')}
                    {bg(selected, row, col)}
                    {completed && completed.some(c => !c.done && c.type === 'row' && c.index === row) ? DorL() : ''}
                    {completed && completed.some(c => !c.done && c.type === 'col' && c.index === col) ? DorL() : ''}"
								...
								>
									{#if $sudokuStore.sudoku[index].number === ''}
										<div class="w-full h-full grid grid-rows-3 grid-cols-3">
											{#each Array(9) as _, i}
												<div class="flex justify-center items-center text-xs {Color($themeColor,'text','700')}">
                          <span
														class="
															transition-opacity
															{
		...
		<div class="flex flex-col gap-5 items-center">
				...
				{#each Array(9) as _, i}
					<button
						class="
							...
							{commonButtonClass}
							{isNumberFullyUsed($sudokuStore.sudoku,(i + 1).toString()) ? 'opacity-50 cursor-not-allowed' : ''}
							{keepMode && selectedNumber === (i + 1).toString() ? 'border-4' : ''}
						"
						disabled={isNumberFullyUsed($sudokuStore.sudoku,(i + 1).toString())}
						style="width: {$sudokuStore.displaySize}px; height: {$sudokuStore.displaySize}px;"
						on:click={() => {
							const [row, col] = selected;
							if (keepMode) {
								if (selectedNumber === (i + 1).toString()) {
									selectedNumber = '';
								} else {
									selectedNumber = (i + 1).toString();
								}
							} else {
								if (isMemo) {
									updateMemo(row, col, (i + 1).toString());
								} else {
									updateCell(row, col, (i + 1).toString());
								}
							}
						}}
					>
			...
			<div class="flex gap-1">
				<button
					class="{commonButtonClass} relative"
					style="width: {$sudokuStore.displaySize}px; height: {$sudokuStore.displaySize}px;"
					on:click={() => $sudokuStore.isMemo = !$sudokuStore.isMemo}
				>
					<Icon
						icon="lucide:pencil"
						class="w-4 h-4 md:w-6 md:h-6 transition-[opacity,transform] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 {$sudokuStore.isMemo ? 'opacity-0 rotate-180 scale-0' : 'opacity-100 rotate-0 scale-100'}"
					/>
					<Icon
						icon="lucide:clipboard-pen"
						class="w-4 h-4 md:w-6 md:h-6 transition-[opacity,transform] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 {$sudokuStore.isMemo ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-180 scale-0'}"
					/>
				</button>
				<button
				...
				<button
					class="{commonButtonClass} relative"
					style="width: {$sudokuStore.displaySize}px; height: {$sudokuStore.displaySize}px;"
					on:click={() => keepMode = !keepMode}
				>
					<Icon
						icon="lucide:lock-open"
						class="w-4 h-4 md:w-6 md:h-6 transition-[opacity,transform] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 {keepMode ? 'opacity-0 rotate-180 scale-0' : 'opacity-100 rotate-0 scale-100'}"
					/>
					<Icon
						icon="lucide:lock"
						class="w-4 h-4 md:w-6 md:h-6 transition-[opacity,transform] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 {keepMode ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-180 scale-0'}"
					/>
				</button>
			</div>
		</div>
